...
export default function Component({ className = '' }: { className?: string }) {
    ...
    const handleCellClick = (row: number, col: number) => {
        ...
        if (quickPlayMode) {
          // Quick Play Mode - Reveal only clicked tile initially
          newBoard[row][col].isRevealed = true;
          const isBomb = newBoard[row][col].isBomb;
          setBoard(newBoard);
          ...
          if (isBomb) {
            ...
            setRoundResults((prevResults) => [
              ...prevResults,
              { round: prevResults.length + 1, timesDied: numDeaths + 1, timesClicked, rows, cols, bombs },
            ]);
            toast(`Too bad, you died on the ${timesClicked + 1} click`);
          } else {
            // Check for win condition in quick play mode
            if (checkWin(newBoard)) {
              setQuickPlayMode(false); // Exit quick play after win
              toast('Congratulations! You won in quick play!');
            }
          }
          return;
        }

        // Regular Mode - Handle clicked cell
        const clickedCell = newBoard[row][col];
        if (clickedCell.isBomb) {
          setGameOver(true);
          revealAll();
        } else {
          revealTile(newBoard, row, col); // Replace with your logic for revealing tiles and surrounding safe tiles
        }

        setBoard(newBoard);
        // Check for win condition in regular mode (assuming you have a checkWin function)
        if (checkWin(newBoard)) {
