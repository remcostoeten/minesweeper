...
const MenuItem =  ({ icon, label, href } : MenuItemProps) => {
 ...
 const getCurrentUrl = () => {
    ...
    const isActive = getCurrentUrl() === href;
   ...
   return (
    <Link href={href} passHref className={`${linkClasses}`}>
      ...
      <span className="flex items-center gap-2"><HomeIcon/>{label}</span>
  )}
    </Link>
    );
  }
const Sidebar = () => {
 const menuItems: MenuItemProps[] = [
    ...
    {
      label: "Theme ui",
      icon:   <svg xmlns="http://www.w3.org/2000/svg" x="0" y="0" viewBox="0 0 100 125">
