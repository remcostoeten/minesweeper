  ...
  return (
    <Link href={href} passHref className={`${linkClasses}`}>
        ...
        <span className="flex items-center gap-2 relative pl-8">
          <span className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2">{icon}</span>
          {label}
        </span>
    </Link>
  )
...
const Sidebar = () => {
  const menuItems: MenuItemProps[] = [
    ...
    },
    {
      label: "Theme ui",
      icon: (
        <svg fill="currentColor" xmlns="http://www.w3.org/2000/svg" width={25} height={25} x="0" y="0" viewBox="0 0 100 125">
